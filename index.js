...
function customEntries(obj) {
	...
	} const entriesList = Object.entries(obj);
	return entriesList;
...
return "Помилка: Вхідний аргумент має бути масивом."
	...
	const entriesList = entries.map(([key, value]) => {
			if (typeof key === "number") {
				key = key.toString();
			}
			return [key, value] });
			const obj = Object.fromEntries(entriesList)
			return obj;
...
function keysAndValues(obj) { if (typeof obj !== 'object') {
	return null
}
 ...
 */
	function checkAndPreventExtensions(obj) { if (typeof obj !== 'object') {
		return false
	}
...
function getObjectValuesSum(obj) {
	...
	let sum = objValue.reduce((num, value) => num + value, 0);
	return sum;
